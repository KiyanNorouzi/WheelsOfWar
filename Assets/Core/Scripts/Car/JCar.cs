...
public class JCar : MonoBehaviour {	
	...
	public bool MotorRunning {
        get { return true; } //return audioSource.isPlaying; }
	}
    ...
    public float startParticleRPM = 300;
    ...
    public Transform wheelBRB; // connect to Back Right Wheel transform
    public Transform wheelBLB; // connect to Back Left Wheel transform
    [Space(1.2f)]
    ...
    public AnimationCurve powerAll = AnimationCurve.Linear(0.0f, 5000.0f, 8000f, 0.0f);
	...
	*/
    ...
    {
        if (wheels == null || wheels.Length <= index)
            return null;
    ...
    public int GetOnGroundWheels()
    {
        int n = 0;
        for (int i = 0; i < wheels.Length; i++)
        {
            if (wheels[i].col.isGrounded)
                n++;
        }
	...
	WheelData SetWheelParams(Transform wheel, float maxSteer, JWheelType wheeltype) {
        ...
        go.transform.rotation = wheel.rotation; // match wheel rot
	...
	void Start () {
        ...
        wheels = (extraWheel)?new WheelData[6]:new WheelData[4];
        ...
        if (extraWheel)
        {
            wheels[4] = SetWheelParams(wheelBRB, 0.0f, JWheelType.Back);
            wheels[5] = SetWheelParams(wheelBLB, 0.0f, JWheelType.Back);
        }
		...
		foreach (WheelData w in wheels) {
            ...
            fc.extremumSlip = frictionSetting.baseData.fwd.extermumSlip;
            fc.extremumValue = frictionSetting.baseData.fwd.extermumValue;
            fc.asymptoteSlip = frictionSetting.baseData.fwd.asymptoteSlip;
            fc.asymptoteValue = frictionSetting.baseData.fwd.asymptoteValue;
            col.forwardFriction = fc;
    ...
    public struct FrictionStruct
    {
        ...
        public struct BaseStruct
        {
            ...
            {
                public float extermumSlip;
                public float extermumValue;
                public float asymptoteSlip;
                public float asymptoteValue;
            }

            public DetailStruct fwd;
            public DetailStruct swy;

        }

        public BaseStruct baseData;
    ...
    public int wheelsRPM;
    public FrictionStruct frictionSetting;

    private WheelFrictionCurve fc;
	public void HandleMotor(float steer, float accel) {
		float delta = Time.fixedDeltaTime;

        float newAccel = accel > 0 ? (accel * steerDowner) : (accel * -steerDowner);
        newAccel = accel>0?Mathf.Clamp(newAccel,0,1):Mathf.Clamp(newAccel,-1,0);
        accel = accel - newAccel;
		...
		if (accel < 0.0f)
        {
            ...
            if (backLampsGameObjects != null)
                backLampsGameObjects.SetActive(true);

            if (currentGear > 0)
            {
                accel = 0;
                int length = currentGear;
        ...
        else if (backLampsGameObjects != null)
        {
            backLampsGameObjects.SetActive(false);
        }
		...
		foreach (WheelData w in wheels) {
            ...
            // sidewaysFriction
            fc = col.sidewaysFriction;
            fc.stiffness += accel > 0 ? (frictionSetting.speed * Time.deltaTime) : 0;
            fc.stiffness -= Mathf.Abs(steer) > 0 ? (frictionSetting.steerDown * Time.deltaTime) : 0;
            fc.stiffness = Mathf.Clamp(fc.stiffness,frictionSetting.limit.x, frictionSetting.limit.y);
            col.sidewaysFriction = fc;
        ...
        if (Mathf.Abs(motorRPM - idleRPM) <= startParticleRPM && calcRPM > motorRPM)
        {
            PlayStartParticles();
        }
        else
        ...
        //Debug.Log("Gear=" + currentGear + ", RPM=" + motorRPM);
    ...
    private void StopStartParticles()
    {
        if (startParticles == null || startParticles.Length == 0)
            return;

        if (startParticles[0].enableEmission)
        {
            for (int i = 0; i < startParticles.Length; i++)
                startParticles[i].enableEmission = false;
        }
    }

    private void PlayStartParticles()
    {
        if (startParticles == null || startParticles.Length == 0)
            return;
        ...
        if (!startParticles[0].enableEmission)
        {
            for (int i = 0; i < startParticles.Length; i++)
                ...
                startParticles[i].enableEmission = true;
        }
    }	
}
